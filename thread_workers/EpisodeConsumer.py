        ...
        try:
            ...
            for item in root.findAll("item"):
                ...
                episode_message = {
                    ...
                    "is_explicit": self.to_bool(item.explicit.get_text(), kafka_message['is_explicit'])
                    if item.explicit else kafka_message['is_explicit'],
                    "publisher": item.author.get_text()
                    if item.author else kafka_message['publisher'],
                    # "image_url": record['artwork_thumbnail'],
                    "description_cleaned": ProcessText.return_clean_text(item.description.get_text())
                    if item.description else '',
                    "title_cleaned": ProcessText.return_clean_text(item.title.get_text())
                    if item.title else '',
                    "readability": 0,
                    ...
                    "publish_date": int(parser.parse(item.pubDate.get_text()).timestamp())
                    if item.pubDate else None}

                Episode(**episode_message)
                # Check for Previous Instance in Redis
                previous_episode_uuid = self.redis_cli.get(record_hash)
                ...
                if previous_episode_uuid == episode_message['episode_uuid']:
                    raise TypeError(
                        "File {} is a duplicate to: {}.".format(episode_message['episode_url'], previous_episode_uuid))
                ...
                # Set entry in Redis
                    ...
                    self.redis_cli.set(record_hash, episode_message['episode_uuid'])
